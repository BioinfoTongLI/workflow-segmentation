...
def track(stem:str, centroids_p:str, cell_volumn_threshold:float=15000):
    ...
    tracked_df = tp.link(cells_df, 25, memory=4, pos_columns=['centroid-0', 'centroid-1', 'centroid-2'])
    plt.figure()
    ...
    filtered_tracked_df = tp.filtering.filter_stubs(tracked_df, threshold=15)
    plt.figure()
